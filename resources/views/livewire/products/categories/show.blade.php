                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                        ...
                        @foreach ($subcategories as ['category' => $subcategory, 'image' => $image])
                            <a href="{{ route('categories.show', $subcategory) }}" wire:key="sub-{{ $subcategory->id }}"
                                class="flex flex-col bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md hover:border-blue-300 transition-all group overflow-hidden">
                                <img src="{{ $image ?? $placeholder }}" alt="{{ $subcategory->name }}"
